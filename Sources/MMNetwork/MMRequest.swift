...
public struct MMNetworkConfig {
    ...
    public static var backgroundTaskHandleTimeInterval = 3.0
}
...
extension MMRequest {
    ...
    private func popTask() {
        DispatchQueue.main.asyncAfter(deadline: .now() + MMNetworkConfig.backgroundTaskHandleTimeInterval) { //给返回过去的结果一些处理时间
